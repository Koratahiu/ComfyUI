                ...
                if any(output_embeds_info_batch):
                    ...
                    for batch_idx, prompt_embeds_list in enumerate(output_embeds_info_batch):
                        ...
                        for seq_idx, embed_tensor in prompt_embeds_list:
                            ...
                            logging.info(f"EMBEDDING_DEBUG: Applying embedding with shape {embed_tensor.shape} at index {final_seq_idx}.")
                            logging.info(f"EMBEDDING_DEBUG: Sample values: {embed_tensor.flatten()[:4]}")
...
class SDClipModel(torch.nn.Module, ClipTokenWeightEncoder):
    ...
    def forward(self, tokens):
        ...
        # In the line below, I have removed the 'embeds_info' argument to fix the TypeError.
...
def load_embed(embedding_name, embedding_directory, embedding_size, embed_key=None):
    ...
    if embed_out is None:
        if 'string_to_param' in embed:
            ...
            # Check for different embedding key conventions, prioritizing the specific SD1.5 format.
            ...
            # 2. Generic '<text_encoder>_out' convention (SDXL, SD3, T5, FLUX, etc.)
